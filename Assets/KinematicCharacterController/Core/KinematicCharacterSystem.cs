...
#pragma warning disable 0162
            ...
            for (int i = 0; i < physicsMoversCount; i++)
            {
				...
				Quaternion safeRotation = mover.TransientRotation.normalized;

				// Если кватернион стал нулевым (invalid), используем текущее вращение транформа
				if (safeRotation.x == 0 && safeRotation.y == 0 && safeRotation.z == 0 && safeRotation.w == 0)
				{
					safeRotation = mover.Transform.rotation;
				}
...
#pragma warning restore 0162
        ...
        public static void PostSimulationInterpolationUpdate(float deltaTime)
        {
			...
			for (int i = 0; i < PhysicsMovers.Count; i++)
			{
				...
				{
					// 1. Исправляем InitialTickRotation перед установкой
					Quaternion initRot = mover.InitialTickRotation;
					float sqrNormInit = initRot.x * initRot.x + initRot.y * initRot.y + initRot.z * initRot.z + initRot.w * initRot.w;

					if (sqrNormInit < 0.001f) initRot = Quaternion.identity;
					else initRot = Quaternion.Normalize(initRot);

					mover.Rigidbody.position = mover.InitialTickPosition;
					mover.Rigidbody.rotation = initRot; // Теперь здесь всегда валидный кватернион

					mover.Rigidbody.MovePosition(mover.TransientPosition);

					// 2. Исправляем TransientRotation перед MoveRotation
					Quaternion transRot = mover.TransientRotation;
					float sqrNormTrans = transRot.x * transRot.x + transRot.y * transRot.y + transRot.z * transRot.z + transRot.w * transRot.w;

					if (sqrNormTrans < 0.001f) transRot = Quaternion.identity;
					else transRot = Quaternion.Normalize(transRot);

					mover.Rigidbody.MoveRotation(transRot);
				}
				else
				{
					// Для порядка нормализуем и здесь
					mover.Rigidbody.position = mover.TransientPosition;
